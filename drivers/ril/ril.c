...
static ssize_t show_power_state(struct device *class,
		struct device_attribute *attr, char *buf);
static ssize_t store_power_state(struct device *class, struct attribute *attr,
		const char *buf, size_t count);
static ssize_t show_reset_state(struct device *class,
		struct device_attribute *attr, char *buf);
static ssize_t store_reset_state(struct device *class, struct attribute *attr,
		const char *buf, size_t count);
static ssize_t show_ulpi_reset_state(struct device *class,
		struct device_attribute *attr, char *buf);
static ssize_t store_ulpi_reset_state(struct device *class,
		struct attribute *attr, const char *buf, size_t count);
static ssize_t show_proxi_state(struct device *class,
		struct device_attribute *attr, char *buf);
static ssize_t store_proxi_state(struct device *class, struct attribute *attr,
		const char *buf, size_t count);
static ssize_t show_sar3g_state(struct device *class,
		struct device_attribute *attr, char *buf);
static ssize_t store_sar3g_state(struct device *class, struct attribute *attr,
		const char *buf, size_t count);
static ssize_t show_3g_available(struct device *class,
		struct device_attribute *attr, char *buf);
...
static int add_ril_files()
{
	...
	dev = device_create(ril_class, NULL, MKDEV(ril_major, ril_minor), NULL,
			"ril_files");
	if (dev <= 0) {
...
void set_wakeup_source(void *dev_id)
{
	int sim_irq = gpio_to_irq(GPIO_SIM_PIN);
	int mod_irq = gpio_to_irq(GPIO_MODEM_WAKEUP);
	if (is3G) {
		tegra_set_lp0_wake_type(sim_irq, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
		enable_irq_wake(sim_irq);

		tegra_set_lp0_wake_type(mod_irq, IRQF_TRIGGER_FALLING);
...
static int init_output_gpio(void)
{
	...
	if (rc = -16) {
		...
		rc = gpio_direction_output(GPIO_ULPI_Reset_PIN, gpio_get_value(
				GPIO_ULPI_Reset_PIN));
		if (rc < 0) {
